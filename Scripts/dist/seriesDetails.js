...
function displaySeriesDetails(series) {
    ...
    const backdropImg = document.getElementById('series-backdrop');
    ...
    networksContainer.textContent = series.networks?.length
        ...
        : 'N/A';
    // Production
    displayData('series-production-countries', series.production_countries?.map(c => c.name).join(', '));
    displayData('series-spoken-languages', series.spoken_languages?.map(l => l.english_name).join(', '));
    // Homepage link
