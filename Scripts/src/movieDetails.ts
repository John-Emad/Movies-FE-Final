...
function displayMovieDetails(movie: MovieDetails): void {
    ...
    const backdropImg = document.getElementById('movie-backdrop') as HTMLImageElement;
    ...
    // Production
    displayData('movie-production-countries', 
        movie.production_countries?.map(c => c.name).join(', '));
    displayData('movie-spoken-languages', 
        movie.spoken_languages?.map(l => l.english_name).join(', '));

    // Collection
    displayData('movie-collection', 
